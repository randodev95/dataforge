...
async fn test_engine_dag_lifecycle() {
    ...
    let task2 = create_test_task(
        ...
        muscle.clone(),
        vde.clone(),
        vec!["stg_users".to_string()],
    );

    // Run DAG
    let _ = filler
        .run_dag(vec![task1.clone(), task2.clone()])
        .await
        .unwrap();
    ...
    let hash1 = filler
        .state_store
        .get_hash_by_name(env_name, "stg_users")
        .unwrap()
        .unwrap();
    let hash2 = filler
        .state_store
        .get_hash_by_name(env_name, "dim_users")
        .unwrap()
        .unwrap();
