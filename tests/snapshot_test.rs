...
async fn test_snapshot_scd2_logic() {
    ...
    materializer
        .materialize(env, model, &hash1, &exec_id1, sql1)
        .await
    ...
    // Verify 1
    {
        ...
        let eager_snapshot =
            deltalake::kernel::EagerSnapshot::try_new(log_store.as_ref(), Default::default(), None)
                .await
                .unwrap();
        ...
        muscle
            .ctx
            .register_table("verify_init", Arc::new(table_provider))
            .unwrap();
        let count_df = muscle
            .ctx
            .sql("SELECT COUNT(*) FROM verify_init")
            .await
            .unwrap();
        let count_results = count_df.collect().await.unwrap();
        let count = count_results[0]
            .column(0)
            .as_any()
    ...
    materializer
        .materialize(env, model, &hash2, &exec_id2, sql2)
        .await
    ...
    muscle
        .ctx
        .register_table("verify_scd2", Arc::new(table_provider))
        .unwrap();
    ...
    let count_df = muscle
        .ctx
        .sql("SELECT COUNT(*) as count FROM verify_scd2")
        .await
        .unwrap();
    let count_results = count_df.collect().await.unwrap();
    let count = count_results[0]
