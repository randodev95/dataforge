...
async fn test_yaml_data_contracts() {
    ...
    // 1. Setup project
    fs::write(root.join("config.yaml"), "name: contract_test\nstorage: rocksdb\n").unwrap();
...
";
    ...
    titan_engine::cli::handle_pipeline(root.clone(), "dev".to_string(), None, false, false).await.unwrap();

    // 6. Run tests - should FAIL
    let result = handle_test(root.clone(), "dev".to_string()).await;
    assert!(result.is_err(), "Expected tests to fail due to unique and not_null violations");
