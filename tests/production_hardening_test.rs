...
use titan_engine::project::{ModelColumn, OnSchemaChange};
use titan_engine::{Filler, LogicHash, ModelTask, Muscle, StateStore, VDE};
...
async fn test_incremental_and_contracts() {
    ...
    muscle
        .register_csv("raw_data", csv1_path.to_str().unwrap())
        .await
        .unwrap();
    muscle
        .execute("CREATE VIEW raw_view AS SELECT * FROM raw_data")
        .await
        .unwrap();
    ...
    muscle
        .register_csv("raw_data_v2", csv2_path.to_str().unwrap())
        .await
        .unwrap();
    muscle
        .execute("CREATE OR REPLACE VIEW raw_view AS SELECT * FROM raw_data_v2")
        .await
        .unwrap();
    ...
    let df = muscle
        .ctx
        .sql("SELECT * FROM dev_inc_model ORDER BY id")
        .await
        .unwrap();
    ...
    assert!(
        err_msg.contains("missing column val"),
        "Error message should mention missing column val, got: {}",
        err_msg
    );
