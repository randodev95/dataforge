...
use crate::{StateStore, Muscle, VDE, ModelMetadata};
use crate::materialize::{Materializer, Materialization};
use std::sync::Arc;
...
impl ModelTask {
    pub async fn resolve_parents(&self) -> Result<Vec<LogicHash>> {
        ...
        for name in &self.parent_names {
            ...
            futures.push(tokio::task::spawn_blocking(move || {
                match state_store.get_hash_by_name(&env, &name) {
                    ...
                    Ok(None) => Err(TitanError::DependencyNotFound(name, env)),
        ...
        for f in futures {
            parent_hashes.push(f.await.map_err(|e| TitanError::ExecutionError(e.to_string()))??);
        }
    ...
    fn validate_contract(&self, df_schema: &datafusion::common::DFSchema) -> Result<()> {
        ...
        for col in &self.columns {
            let field = df_schema.field_with_name(None, &col.name)
                .map_err(|_| TitanError::ValidationError(format!("Contract violation: model {} is missing column {}", self.name, col.name)))?;
    ...
    async fn execute_internal(&self, start_time: std::time::Instant) -> Result<(LogicHash, u128)> {
        ...
        let is_inc_run = self.materialization == Materialization::Incremental && self.state_store.get_metadata_by_name(&self.env, &self.name).unwrap_or(None).is_some();
        ...
        let df = self.muscle.ctx.sql(titan_sql.as_str()).await
        ...
        strategy.materialize(&self.env, &self.name, &logic_hash, &self.exec_id, titan_sql.as_str()).await?;
        ...
        match self.target_type.as_str() {
            "delta" => {
                let table_path = if self.materialization == Materialization::Incremental {
                    self.project_root.join(format!("tables/{}/{}", self.env, self.name))
                } else {
                    self.project_root.join(format!("snapshots/{}/{}", self.env, self.name))
                };
                if table_path.exists() {
                    self.muscle.register_delta(&target_name, table_path.to_str().unwrap()).await?;
                }
            }
            "parquet" => {
                let table_path = self.project_root.join(format!("tables/{}/{}.parquet", self.env, self.name));
                if table_path.exists() {
                    self.muscle.register_parquet(&target_name, table_path.to_str().unwrap()).await?;
                }
            }
            "csv" => {
                let table_path = self.project_root.join(format!("tables/{}/{}.csv", self.env, self.name));
                if table_path.exists() {
                    self.muscle.register_csv(&target_name, table_path.to_str().unwrap()).await?;
                }
            }
            _ => {
                self.muscle.ctx.register_table(&target_name, df.into_view())
                    .map_err(|e| TitanError::ExecutionError(e.to_string()))?;
            }
        }

        // Also register a "stable" name for downstream tasks if they are in the same context
        // NOTE: In a real FAANG setup, we'd use a more robust reference system, but for now 
        // we'll keep the env_name as the stable alias.
        let stable_name = format!("{}_{}", self.env, self.name);
        let df = self.muscle.ctx.table(&target_name).await.map_err(|e| TitanError::ExecutionError(e.to_string()))?;
        self.muscle.ctx.register_table(&stable_name, df.into_view())
            .map_err(|e| TitanError::ExecutionError(e.to_string()))?;
        ...
        self.state_store.put_value(&logic_hash, titan_sql.as_str().to_string())
            .map_err(|e| TitanError::StateError(e.to_string()))?;
...
impl Filler {
    pub fn new(state_store: StateStore, project_root: &std::path::Path, concurrency: usize, cancellation_token: CancellationToken) -> Self {
        Self {
    ...
    pub async fn run_dag(&self, tasks: Vec<ModelTask>) -> Result<crate::artifacts::RunResults> {
        ...
        let name_to_task: HashMap<&str, &ModelTask> = tasks.iter().map(|t| (t.name.as_str(), t)).collect();

        loop {
            let ready_tasks: Vec<&str> = pending.iter()
                .filter(|name| {
                    let task = name_to_task[**name];
                    task.parent_names.iter().all(|p| !name_to_task.contains_key(p.as_str()) || completed.contains(p.as_str()))
                })
                .copied()
                .collect();
