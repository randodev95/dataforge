...
impl super::Connector for ObjectStoreConnector {
    async fn register(
        ...
        let url = Url::parse(&format!("{}://{}", config.source_type, name)).map_err(|e| {
            TitanError::ValidationError(format!("Invalid URL for source {name}: {e}"))
        })?;

        match config.source_type.as_str() {
            "s3" => {
                let bucket = config.bucket.as_ref().ok_or_else(|| {
                    TitanError::ValidationError("Bucket required for S3".to_string())
                })?;
            ...
            "gcs" => {
                let bucket = config.bucket.as_ref().ok_or_else(|| {
                    TitanError::ValidationError("Bucket required for GCS".to_string())
                })?;
            ...
            "azure" => {
                let bucket = config.bucket.as_ref().ok_or_else(|| {
                    TitanError::ValidationError("Container required for Azure".to_string())
                })?;
