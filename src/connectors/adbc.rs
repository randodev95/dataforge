...
impl super::Connector for AdbcConnector {
    async fn register(&self, _ctx: &SessionContext, name: &str, config: &SourceConfig) -> Result<()> {
        info!(source = %name, "Registering ADBC source");
        
        let driver = config.driver.as_ref().ok_or_else(|| TitanError::ValidationError("ADBC driver path required".to_string()))?;
        let uri = config.connection_string.as_ref().ok_or_else(|| TitanError::ValidationError("Connection string required".to_string()))?;
